...
// The model class is derived from the AnimatedTickingGeoModel with the template argument(<>) set to the previously made
// entity class
public class ExampleEntityZModel extends AnimatedTickingGeoModel<GeoExampleEntityZ> {

    // This method should return a ResourceLocation for the animation file based on the entity state
    @Override
    public ResourceLocation getAnimationFileLocation(GeoExampleEntityZ entity) {
    ...
    @Override
    public ResourceLocation getModelLocation(GeoExampleEntityZ entity) {
    ...
    @Override
    public ResourceLocation getTextureLocation(GeoExampleEntityZ entity) {
    ...
    @Override
    public void setLivingAnimations(GeoExampleEntityZ entity, Integer uniqueID, AnimationEvent animationEvent) {
        super.setLivingAnimations(entity, uniqueID, animationEvent); // We call the super-function
        IBone head = this.getAnimationProcessor()
            .getBone("head"); // Then we take the head bone
        // We get the model data for an entity
        EntityModelData extraData = (EntityModelData) animationEvent.getExtraDataOfType(EntityModelData.class)
            .get(0);
